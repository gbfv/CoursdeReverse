...
L'encodage des instruction se nomme ISA (Instruction Set Architecture). En \xquatre, une \textit{instruction} se décompose selon le format\footnote{Plus de détails à \url{http://ref.x86asm.net/coder32.html} par exemple.} :%TODO dire qu'en bytes bah c'est quasiment arei
\begin{eqnarray*}
...
Les énoncés comme 
\mint{asm}{Text db "Hello word", 0} décrivent le contenu de la mémoire à une position nommée "Text", octet par octet. 
...
\begin{itemize}
	\item des informations pour préparer la zone de l'exécutable,
